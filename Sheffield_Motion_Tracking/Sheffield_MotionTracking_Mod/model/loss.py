...
class LNCC(nn.Module):
    ...
    def forward(self, I, J, device='cuda'):
        '''
        Parameters
        ----------
        ...
        ncc_map = cross / ((I_var * J_var) ** 0.5)

        weight = torch.FloatTensor([1 / I.shape[0] for i in range(I.shape[0])]).to(device)
        ncc_per_frame =  ncc_map.mean(dim=[1, 2, 3]) # Find the average!
        ncc = torch.sum(ncc_per_frame * weight)

        return 1.0-ncc, ncc_per_frame
...
def smooth_loss_simple(disp, image):
    '''
    Calculate the smooth loss. Return mean of absolute or squared of the forward difference of  flow field.
    ...
    loss = torch.mean(torch.sum(torch.abs(d_disp), dim=2, keepdims=True))/2.0
